...
const GetMenuByOwnerId: React.FC = () => {
  ...
  const fetchMenus = async () => {
    ...
    try {
      const response = await fetch("http://192.168.0.225:5000/menu/getmenubyresturantid", {
        method: "POST",
        body: formData,
      });
      if (!response.ok) {
  ...
  const deleteMenu = async (menuid: string) => {
    ...
    try {
      const response = await fetch("http://192.168.0.225:5000/resops/deletemenu", {
        method: "POST",
        body: formData,
      });
      if (!response.ok) {
  ...
  const markOutOfStock = async (menuid: string) => {
    ...
    try {
      const response = await fetch("http://192.168.0.225:5000/ops/fastfilling", {
        method: "POST",
        body: formData,
      });
      if (!response.ok) {
